...
<div class="main-content" x-data="{ isFinishModalOpen: false }">
    ...
    {{-- FORM UTAMA UNTUK MENYELESAIKAN PRODUKSI --}}
    <form id="finishForm" action="{{ route('produksi.productions.complete', $production) }}" method="POST">
        ...
        <div class="card-custom mb-6">
                ...
                <table class="table-custom min-w-[600px]">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center w-48">Hasil Nyata (Selesai)</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <div class="font-bold text-gray-900 text-lg">{{ $item->menu->name }}</div>
                                        <div class="text-xs text-gray-500 font-medium">Kategori: {{ $item->menu->category }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="text-center">
                                <span class="font-black text-xl text-gray-900">{{ $item->target_quantity }}</span>
                            </td>
                            <td>
                                <div class="flex items-center justify-center">
                                    <input 
                                        ...
                                        min="0"
                                        {{ $production->status === 'planned' ? 'disabled' : '' }}
                                        class="w-24 h-12 text-center text-xl font-black bg-gray-50 border-2 border-gray-200 rounded-xl focus:border-[#7b4a24] focus:ring-0 transition disabled:opacity-50">
                                </div>
                            </td>
                        </tr>
                        @endforeach
            ...
            @if($production->status === 'processing')
                <div class="mt-6 flex items-start gap-3 bg-[#fdfaf6] p-4 rounded-xl border border-[#ece7e2]">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-[#ef8d1d] shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg>
                    <p class="text-sm text-[#5c4432] leading-relaxed">
                        <strong class="font-bold text-[#2c1f16]">Catatan:</strong> Ubah angka <span class="font-semibold">Hasil Nyata</span> di atas jika ada porsi yang tumpah/gagal saat proses memasak. Sistem hanya akan memotong bahan baku dan menambahkan stok jadi sesuai dengan jumlah yang benar-benar selesai.
                    </p>
                </div>
            @endif
        </div>
        ...
        <div x-show="isFinishModalOpen" class="fixed inset-0 z-[100] flex items-center justify-center" style="display: none;">
            ...
            <div class="bg-white rounded-3xl p-6 max-w-sm w-full mx-4 relative z-10 shadow-2xl"
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button type="button" @click="document.getElementById('finishForm').submit()" class="flex-1 h-12 rounded-2xl bg-[#2faa39] text-white font-bold hover:bg-[#289631] shadow-lg shadow-green-600/30 transition">Ya, Selesaikan</button>
                </div>
            </div>
        </div>
