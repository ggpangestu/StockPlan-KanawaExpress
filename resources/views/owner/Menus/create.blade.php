...
<div class="space-y-6">
    <div class="bg-white rounded-3xl border border-black/5 shadow-sm p-6">
        <h1 class="text-3xl font-bold text-[#2c1f16]">Create Menu</h1>
        <p class="text-[#5c4432] mt-1">Tambahkan menu baru beserta resep bahan bakunya.</p>
    </div>

    <form action="{{ route('owner.menus.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
            ...
            <div class="grid grid-cols-1 xl:grid-cols-[160px_1fr] gap-6">
                <div x-data="{ preview: null }">
                    ...
                    <div class="w-36 h-36 rounded-3xl border border-dashed border-black/10 bg-black/[0.02] overflow-hidden">
                        ...
                        <template x-if="!preview"><div class="w-full h-full flex items-center justify-center"><span class="text-xs text-[#5c4432] text-center px-3">Upload Image</span></div></template>
                    </div>
                ...
                <div class="space-y-5">
                    <div class="grid grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" class="w-full mt-2 rounded-2xl border border-black/10 px-4 h-11" required>
                        </div>
                        <div>
        ...
        <div class="bg-white rounded-3xl border border-black/5 shadow-sm p-6" 
             x-data="{ 
                ingredients: [ { id: Date.now(), raw_material_id: '', quantity: '' } ],
            ...
            <div class="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p class="text-sm text-[#5c4432] mt-1">Pilih bahan baku dan takaran yang dibutuhkan untuk 1 porsi menu ini.</p>
                </div>
                ...
                <template x-for="(item, index) in ingredients" :key="item.id">
                    <div class="flex items-end gap-4 p-4 rounded-2xl border border-black/10 bg-black/[0.01]">
                        ...
                        <div class="w-1/3">
                            ...
                            <input type="number" step="0.01" :name="`ingredients[${index}][quantity]`" x-model="item.quantity" placeholder="e.g. 15" class="w-full mt-2 rounded-xl border border-black/10 px-4 h-11" required>
                        </div>
        ...
        <div class="flex items-center justify-end gap-3">
            ...
            <button type="submit" class="inline-flex items-center justify-center h-11 px-6 rounded-2xl bg-[#2c1f16] text-white font-medium hover:opacity-90 transition">Save Menu</button>
        </div>
    </form>
</div>
@endsection